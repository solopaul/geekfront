  ...
  <el-col :span="24">
    <el-row>
      ...
      <el-tabs type="border-card" class="keyset-tab" loading="true">
        <el-tab-pane v-for="itempanel in keyset" :key="itempanel.index" :lazy="islazy" style="height:200px;">
          <span slot="label" class="tab-icon">
          <i :class="itempanel.icon"></i> {{ itempanel.title }}
          </span>
          <div v-for="itemline in itempanel.keys" :key="itemline.index">
            <el-button v-for="itemkey in itemline.linekeys" :key="itemkey.index" class="key-btn" :class="itemkey.Class" :icon="itemkey.Icon" @click="handleSetKeyFunc(itemkey)">
              <template v-if="!itemkey.Icon">
                <template v-if="system != '' && itemkey['LangTitle' + system]">
      ...
      <div class="keyset-foot">
        <el-form :inline="true" size="mini">
          ...
          <el-form-item label="功能">
            <el-select v-model="combval" placeholder="选择" @change="combChange" style="width:10rem">
              <el-option
            ...
            <el-input v-model="curkeytext" placeholder="选择" disabled style="width:5rem"></el-input>&nbsp;
            <el-button icon="el-icon-check" @click="setCombFunc">使用</el-button>
          </el-form-item>
          <el-form-item v-if="isdrivermode" label="快捷方式">
            <el-input v-model="shortcutpathshow" placeholder="选择" readonly style="width:10rem">
              <el-button slot="append" icon="el-icon-menu" @click="openFileDlg"></el-button>
            </el-input>&nbsp;
            <el-button icon="el-icon-check" @click="setShortcutFunc">选择</el-button>
          </el-form-item>
        </el-form>
      </div>
    </el-row>
...
export default {
  ...
  methods: {
    ...
    setCombFunc(){
      ...
      var combfuncval = '0x0' + (parseInt(this.combval) + parseInt(this.curkeyval) - parseInt(0x02000000)).toString(16);
